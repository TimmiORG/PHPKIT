...
class pkSetup
{
	...
	var $pathhash = array(
		'password', 'welcome', 'licence', 'options', 'systemcheck', 'database', 'databasecreate', 'createadmin',
		'sitesetting', 'preset', 'finish', 'dbcheck', 'dbcleanup', 'update', 'outofhere', 'uninstall', 'uninstalled'
	);
	...
	var $optionhash = array(
		0 => array(
			'key' => 'leave', 'path' => 'outofhere', 'mode' => '',
		), 1 => array(
			'key' => 'installation', 'path' => 'systemcheck', 'mode' => 'install',
		), 2 => array(
			'key' => 'update', 'path' => 'update', 'mode' => '',
		), 5 => array(
			'key' => 'recoveradmin', 'path' => 'createadmin', 'mode' => 'recoveradmin',
		), 6 => array(
			'key' => 'dbdata', 'path' => 'database', 'mode' => 'dbdata'
		), 7 => array(
			'key' => 'dbcheck', 'path' => 'dbcheck', 'mode' => 'dbcheck',
		), 8 => array(
			'key' => 'dbcleanup', 'path' => 'dbcleanup', 'mode' => 'dbcleanup',
		), 9 => array(
			'key' => 'uninstall', 'path' => 'uninstall', 'mode' => 'uninstall',
		),
	);

	var $steps = array(
		'licence' => 'options', 'systemcheck' => 'database', 'database' => 'databasecreate',
	);
	...
	function __construct()
	{
		...
		pkLoadLang('default');
		pkLoadLang('public');
		$this->pkC = pkConstant(strrev('Ckp'));
		pkLoadLang('setup');
		...
		if ($this->path == 'welcome' || $this->path == 'uninstalled' || ($this->setuppassword && isset(
		$_SESSION[$this->setup_session_key]) && $_SESSION[$this->setup_session_key] === $this->setuppassword))
		{
			$this->path();
		}
		else
	...
	function password()
	{
		...
		$this->body = pkGetSpecialLang('setup_password_description', substr(str_replace(pkDIRROOT, '', pkDIRETC), 0, -1), #dir etc
		                               substr(str_replace(pkDIRROOT, '', pkDIRTEMP), 0, -1), #dir temp
		                               basename(pkFILESQL), basename($this->setuppasswordfile));
		...
		if (in_array($error, array(
		                          'password_invalid', 'writing_failed'
		                     )))
		{
	...
	function createadmin()
	{
		...
		if ($this->ENV->requestmethod('post'))
		{
			...
			{
				$result = $this->SQL->query(($userid ? "UPDATE" : "INSERT INTO") . " " . pkSQLTAB_USER . " SET
					user_name='" . $this->SQL->f($username) . "',
					user_pw='" . $this->SQL->f($password) . "',
					user_status='admin',
					user_email='" . $this->SQL->f($useremail) . "'" . ($userid ? " WHERE user_id='" . $userid . "'" : ",user_nick='" . $this->SQL->f($username) . "',signin='" . pkTIME . "',user_id=1"));
		...
		if (!empty($error))
		{
			$errors = '';
			$errorhash = array(
				1 => 'setup_admin_error_password_empty', 2 => 'setup_admin_error_passwords_not_equal',
				3 => 'setup_admin_error_username_in_use', 4 => 'setup_admin_error_empty_username',
				5 => 'setup_admin_error_wrong_legth_username', 6 => 'setup_admin_error_email_invalid',
				7 => 'setup_admin_error_email_empty', 8 => 'setup_admin_error_data_not_saved'
			);
	...
	function sitesetting()
	{
		...
		// preset
		$site_url = $site_urls = $site_name = $site_title = $site_email = '';
	...
	function preset()
	{
		...
		if ($this->ENV->_post_action('button'))
		{
			...
			$SQL->query("REPLACE INTO " . pkSQLTAB_CONFIG . " (id,value) VALUES
				('version_number','" . $SQL->f(serialize(pkPHPKIT_VERSION)) . "')" . ($this->style_standardid ? ",('site_style','" . $SQL->f(serialize(intval($this->style_standardid))) . "')" : ''));
		...
		$dbtables = array(
			pkSQLTAB_NAVIGATION, pkSQLTAB_NAVIGATION_CATEGORY, pkSQLTAB_SMILIES, pkSQLTAB_FORUM_RANK, pkSQLTAB_CONTENT,
			pkSQLTAB_CONTENT_CATEGORY, pkSQLTAB_POLL_COUNT, pkSQLTAB_POLL_TOPIC, pkSQLTAB_POLL, pkSQLTAB_GUESTBOOK,
			pkSQLTAB_FAQ, pkSQLTAB_FAQ_CATEGORY, pkSQLTAB_FORUM_CATEGORY, pkSQLTAB_FORUM_FAVORITE, pkSQLTAB_FORUM_INFO,
			pkSQLTAB_FORUM_POST, pkSQLTAB_FORUM_NOTIFY, pkSQLTAB_FORUM_THREAD
		);
	...
	function preset_adminnavigation()
	{
		$S = $this->SQL;
		$keys = array(
			'id', 'pid', 'sorting', 'lkey', 'lscope', 'target', 'lnkpath', 'lnkmode', 'lnkadd', 'permission'
		);
		$values = array(
		);
	...
	function preset_config_groups()
	{
		...
		{
			$values[] = "'" . $S->f($item['id']) . "','" . $S->f($item['sorting']) . "','" . $S->f(
				$item['lkey']) . "','" . $S->f($item['lscope']) . "'";
		}
...
#END methode update_exists
	...
	private function create_sql_file($database, $sqlhost, $sqluser, $sqlpass, $sqlprefix, $sqltables)
	{
		...
		$lines = array(
			"<?php", "if(defined('pkPHPKIT_INSTALLED')) return;", "", "define('pkPHPKIT_INSTALLED',1);", "",
			"#connection data", "define('pkSQLDATABASE', '" . addslashes($database) . "');",
			"define('pkSQLHOST', '" . addslashes($sqlhost) . "');",
			"define('pkSQLUSER', '" . addslashes($sqluser) . "');",
			"define('pkSQLPASS', '" . addslashes($sqlpass) . "');",
			"define('pkSQLPREFIX', '" . addslashes($sqlprefix) . "');", "", "#table aliases"
		);
		#lines = array
