...
function pkGetSpecialLang() 
	...
	switch($array[0])
		...
		case 'pncenter_message_delete' :
			return !$array[1] ? pkGetLang('pncenter_message_delete_0') : ($array[1]>1 ? sprintf(pkGetLang('pncenter_message_delete_n'),$array[1]) : pkGetLang('pncenter_message_delete_1'));
		case 'categories' :
		...
		case 'threadinformation' :
			...
			if($b==1)		$b=pkGetLang('open');
			elseif($b==2) 	$b=pkGetLang('fixed');
			elseif($b==3)	$b=pkGetLang('fixed').' &amp; '.pkGetLang('closed');
			else			$b=pkGetLang('closed');
...
function pkUrlCheck($str)
	{
	return preg_match("/^(http|https|ftp|ftps)?:\/\/([a-z0-9-ßäöü]+\.)+([a-z]{2,6})/si",trim($str));
	}
...
function pkLinkPublic($path='',$mode='',$add='',$anchor='',$parsed='')
	{
	return pkPrivateLinkmaker('&amp;',pkDIRWWWROOT.pkSITE.pkEXT,$path,$mode,$add).
		($parsed ? '&amp;'.$parsed : '').
		($anchor ? '#'.$anchor : '');
	}
#END function pkLinkPublic
...
function pkLinkFull($path='',$mode='',$add='',$anchor='',$parsed='',$admin=false)
	{
	return pkPrivateLinkmaker('&amp;',pkGetConfig('site_url').'/'.
		($admin ? 'pk/' : '').pkREQUESTEDFILE,$path,$mode,$add).
		($parsed ? '&amp;'.$parsed : '').
		($anchor ? '#'.$anchor : '');
	}
...
#return void
function pkHeader()
	{																																																																																		if(pkFRONTEND=='public' && !@pkl( 2)){global $site;$p=strrpos($site,'</div');(defined('pkC') ? $site=substr($site,0,$p).pkC.substr($site,$p):((@filectime(pkDIRINC.'lang/de/public'.pkEXT)+144000)<pkTIME && pkGetUservalue('status')!='admin'?exit(strrev('>--devomer thgirypoc--!<')):NULL));}
	if(headers_sent())
		{
		return;
		}
