...
$contentid = (isset($_REQUEST['contentid']) && intval($_REQUEST['contentid'])>0) ? intval($_REQUEST['contentid']) : $contentid;
...
$sqlcommand="SELECT ".pkSQLTAB_CONTENT.".* ".
	$sqlcommand."
...
if(is_array($contentinfo_cache))
	...
	foreach($contentinfo_cache as $contentinfo)
		...
		$news_text=$BBCODE->parse($news_text,$contentinfo['content_html'],$contentinfo['content_ubb'],$contentinfo['content_smilies'],1);

		if(!$contentid && eregi("<break>",$contentinfo['content_text']))
			{
...
if($contentid)
	...
	if($counter>$config['content_epp2'])
		{
		$page_link=sidelinkfull($counter,$config['content_epp2'],$entries,"include.php?path=news&catid=".$catid,"sitebodysmall");
		}
