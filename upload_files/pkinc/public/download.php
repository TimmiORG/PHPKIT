...
if(isset($_REQUEST['download']) && $_REQUEST['download']=='go')
	...
	if(getrights($config['content_downloadstatus'])!="true" || getrights($contentcatinfo['contentcat_rights'])!="true") 
		{
...
if($userinfo=$SQL->fetch_array($SQL->query("SELECT user_id, user_nick FROM ".pkSQLTAB_USER." WHERE user_id='".$contentinfo['content_autorid']."' LIMIT 1")))
	{
...
$content_article_body=$BBCODE->parse($contentinfo['content_text'], $contentinfo['content_html'], $contentinfo['content_ubb'], $contentinfo['content_smilies'],1);
$dl=explode("\n",$contentinfo['content_altdat']);
	
unset($download_links);
unset($download_file);
unset($file_size);
