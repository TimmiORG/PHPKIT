...
if(intval($_REQUEST['postid'])>0)
	...
	if($forumpost['forumpost_autorid']!=pkGetUservalue('id'))
		{
		$forumthread=$SQL->fetch_array($SQL->query("SELECT forumthread_catid FROM ".pkSQLTAB_FORUM_THREAD." WHERE forumthread_id='".$forumpost['forumpost_threadid']."' LIMIT 1"));
...
if(!userrights($forumcat['forumcat_mods']) && ($forumpost['forumpost_autorid']>0 && $forumpost['forumpost_autorid']!=pkGetUservalue('id')))
	{
...
elseif($ACTION==$_POST['delete'] && $_POST['delete_confirm']=="confirmed")
	...
	if($threadcount=$SQL->fetch_array($SQL->query("SELECT forumpost_time, forumpost_autor, forumpost_autorid FROM ".pkSQLTAB_FORUM_POST." WHERE forumpost_threadid='".$forumpost['forumpost_threadid']."' ORDER by forumpost_time DESC LIMIT 1")))
		{
...
if($ACTION==$_POST['save'])
	...
	list($firstpostid)=$SQL->fetch_row($SQL->query("SELECT MIN(forumpost_id) FROM ".pkSQLTAB_FORUM_POST." WHERE forumpost_threadid='".$forumpost['forumpost_threadid']."' LIMIT 1"));
