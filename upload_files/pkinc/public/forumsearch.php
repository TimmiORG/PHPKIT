...
$search_textoption=(isset($_REQUEST['search_textoption']) && intval($_REQUEST['search_textoption'])==1) ? 1 : 0;
$search_useroption=(isset($_REQUEST['search_useroption']) && intval($_REQUEST['search_useroption'])==1) ? 1 : 0;
	...
	case 'result' :
		...
		elseif(!empty($_POST['search_user']))
			...
			$query=$SQL->query("SELECT 
					user_id 
				FROM ".pkSQLTAB_USER." 
				WHERE user_nick".(($_POST['search_useroption']==1) ? "='".$suser."' " : 
					"='".$suser."' OR
			...
			$searchstring.=($searchstring ? " OR (" : '').
				"forumpost_autor".
				(($_POST['search_useroption']==1) ? "='".$suser."' " :
				"='".$suser."' OR
		...
		if(!empty($_POST['search_text']))
			...
			foreach($search_string as $k)
				{
				if(!eregi('or',$k) && strlen($k)<pkGetConfig('search_min_length'))
					{
					$search_text=$ENV->_post('search_text');
					$search_user=$ENV->_post('search_user');
					$userid=$ENV->_request('userid');
		...
		if(!empty($searchstring))
			...
			while($posts=$SQL->fetch_assoc($getpostings))
				...
				if($sqlcommand)
					...
					$sqlcommand.="SELECT forumthread_catid, forumthread_id FROM ".pkSQLTAB_FORUM_THREAD." WHERE (forumthread_id='".$posts['forumpost_threadid']."'";
				...
				if(is_array($post_cache) && is_array($threads_cache))
					...
					foreach($post_cache as $posts)
						...
						if(!empty($threads['forumthread_catid']))
							...
							if(getrights($cats['forumcat_rrights']) || userrights($cats['forumcat_mods'],$cats['forumcat_rrights'])|| userrights($cats['forumcat_user'],$cats['forumcat_rrights']))
								{
								$rposts[]=array($posts['forumpost_id'],$posts['forumpost_threadid']);
								}
							}
						}#END foreach
					}
		...
		if($rshow==1)
			...
			while($forumthread=$SQL->fetch_array($getforumthread))
				...
				$sqlcommand.=','.$forumthread['forumthread_autorid'].','.$forumthread['forumthread_lastreply_autorid'];
				}
			...
			foreach($forumthread_cache as $forumthread)
				...
				if($posts[0]>$forumcat['forumcat_posts'])
					$sidelink=" - ".sidelinksmall($posts[0],$forumcat['forumcat_posts'],"include.php?path=forumsthread&threadid=".$forumthread['forumthread_id'],"small");
				...
				if($forumthread['forumthread_replycount']>=$forumcat['forumcat_threads'] || $forumthread['forumthread_viewcount']>=$forumcat['forumcat_views'])
					$threadstatus.="hot";
				...
				if(!empty($userinfo['user_nick']))
					...
					$thread_last_autor=pkEntities(pkStringCut($forumthread['forumthread_lastreply_autor'],$config['forum_threadautor_cut']));
			...
			while($forumpost=$SQL->fetch_array($getforumpost))
				...
				if($sqlcommand)
					...
					$sqlcommand="SELECT forumthread_id, forumthread_title, forumthread_icon FROM ".pkSQLTAB_FORUM_THREAD." WHERE forumthread_id='".$forumpost['forumpost_threadid']."'";
				...
				if($forumpost['forumpost_autorid']>0)
					...
					if($sqlcommand_user)
						...
						$sqlcommand_user="SELECT * FROM ".pkSQLTAB_USER." WHERE user_id='".$forumpost['forumpost_autorid']."'";
					}
				}
			...
			foreach($forumpost_cache as $forumpost)
				...
				if($forumpost['forumpost_autorid']>0 && array_key_exists($forumpost['forumpost_autorid'],$userinfo_cache))
					{
		...
		if(is_array($forumcat_cache_byname))
			...
			foreach($forumcat_cache_byname as $forumcat)
				...
				if($FORUM->getCategoryRrights($forumcat['forumcat_id']))
					$cat_option.='<option value="'.$forumcat['forumcat_id'].'">'.str_repeat('-',$forumcat['level']).' '.pkEntities($forumcat['forumcat_name']).'</option>';
				}
			}


		$order0=$rorder1=$rshow0=$rshow1=
		$search_useroption1=$search_useroption0='';
