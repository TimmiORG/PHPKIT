...
switch($mode)
	...
	case 'new' :
		...
		while($contentinfo=$SQL->fetch_array($getcontentinfo))
			...
			$content_text = $BBCODE->parse($contentinfo['content_header']." ".$contentinfo['content_text'],1,1,1,1);
	...
	default :
		...
		if(intval($contentinfo['content_autorid'])>0)
			$userinfo=$SQL->fetch_array($SQL->query("SELECT user_id, user_nick FROM ".pkSQLTAB_USER." WHERE user_id='".$contentinfo['content_autorid']."' LIMIT 1"));
		
		if(intval($contentinfo['content_themeid'])>0)
			$contentthemeinfo=$SQL->fetch_array($SQL->query("SELECT * FROM ".pkSQLTAB_CONTENT_THEME." WHERE contenttheme_id='".$contentinfo['content_themeid']."' LIMIT 1"));
			
		$content_time=formattime($contentinfo['content_time'],'','date');
		$content_title=pkEntities($contentinfo['content_title']);
		$contentcat_name=pkEntities($contentcatinfo['contentcat_name']);
			
		if($userinfo['user_id']>0 && $userinfo['user_nick']!='')
			{
		...
		if(eregi('<break>',$contentinfo['content_text']))
			...
			$article_pagelink = pagelink($page_count,1,$page,pkLink('article','','contentid='.$contentinfo['content_id']));
			}
		else
			{
