...
if( intval( pkGetUservalue( 'id' ) ) > 0 )
{
	...
	if(!empty($_GET['error']))
	{
		...
		{
			case 1:
				$forum_favorite_error = pkGetLang('forum_favorites_limit');
				break;

			case 2:
				$forum_favorite_error = pkGetLang('forum_favorites_duplicated');
				break;
		}
	}
	...
	if( isset($_REQUEST[ 'add' ]) && intval( $_REQUEST[ 'add' ] ) > 0 )
	{
		...
		pkHeaderLocation( 'forumsfavorites', '', $error );
	}

	// Modified notification informations
	...
	if( isset($_REQUEST[ 'deleteid' ]) )
	{
		...
		{
			$SQL->query("DELETE FROM " . pkSQLTAB_FORUM_FAVORITE . " WHERE forumfav_threadid = '" . $SQL->i($deleteid) . "' AND forumfav_userid = '" . $SQL->i(pkGetUservalue( 'id' )) . "'");
			$SQL->query("DELETE FROM " . pkSQLTAB_FORUM_NOTIFY . " WHERE forumnotify_threadid = '" . $SQL->i($deleteid) . "' AND forumnotify_userid = '" . $SQL->i(pkGetUservalue( 'id' )) . "'");
		}
