...
switch($mode)
	...
	case 'print' :
		...
		$cat=$SQL->fetch_assoc($SQL->query("SELECT * FROM ".pkSQLTAB_CONTENT_CATEGORY." WHERE contentcat_id='".$info['content_cat']."'"));
		...
		if($info['content_themeid']!=0)
			{
			$theme=$SQL->fetch_assoc($SQL->query("SELECT * FROM ".pkSQLTAB_CONTENT_THEME." WHERE contenttheme_id='".$info['content_themeid']."'"));
		...
		if($info['content_text']!='')
			{
			$content_text=$BBCODE->parse(eregi_replace('<break>','',$info['content_text']), $info['content_html'], $info['content_ubb'], $info['content_smilies'],1);
			}
		...
		#prefix.title.suffix
		$CMS->site_title_set(pkGetLang('print_view_prefix').$info['content_title'].pkGetLang('print_view_suffix'));
		...
		if($contentvote && $contentid)
			...
			if($info[0]=='0')
				...
				if($contentinfo['content_rating']>0)
					{
					$i=(($contentinfo['content_rating']*$contentinfo['content_rating_total'])+$contentvote)/($contentinfo['content_rating_total']+1);
					}
				else
					{
					$i=$contentvote;
