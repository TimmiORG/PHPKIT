...
//presets
$newuser = $newemail = $newemail2 = $action = '';
$error = 0;
...
if( !empty( $_POST[ 'refuse' ] ) && $ACTION == $_POST[ 'refuse' ] )
{
	pkHeaderLocation( );
}

if( ( !empty( $_POST[ 'accept' ] ) && $ACTION == $_POST[ 'accept' ] && $config[ 'user_disclaimer' ] == 1 ) ||
    $config[ 'user_disclaimer' ] == 0 || ( !empty( $_POST[ 'send' ] ) && $ACTION == $_POST[ 'send' ] ) || ( !empty(
$_POST[ 'disclaimer' ] ) && 1 == $_POST[ 'disclaimer' ] ) )
{
	$disclaimer = 1;
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					if(intval( pkGetConfig( 'user_activate' ) ) === 0 || intval( pkGetConfig( 'user_registry' ) ) === 1 || intval( pkGetConfig( 'user_registry' ) ) === 0)
					{
						$user_activate_status = 0;
					}
					...
					$SQL->query( "INSERT INTO " . pkSQLTAB_USER . "
						...
						('" . $SQL->f( $uid ) . "','" . $SQL->f( $newuser ) . "','" . $SQL->f( $newuser ) . "',
						 ...
						 '" . pkTIME . "','" . pkTIME . "','" . $SQL->f( $user_activate_status) . "')" );
					$info = $SQL->insert_id( );
					...
					{
						$mail_text = pkGetSpecialLang( 'registration_mail_body_novalidation', $newuser, pkGetConfig( 'site_name' ), $mail_addtext, $newuser, $password, pkGetConfig( 'site_name' ), pkGetConfig( 'site_url' ) );
					}
					else
					{
						$mail_text = pkGetSpecialLang( 'registration_mail_body', $newuser, pkGetConfig( 'site_name' ), $mail_addtext, $newuser, $password, $uid, pkGetConfig( 'site_name' ), pkGetConfig( 'site_url' ) );
					}
					...
					if( mailsender( $receiver_alias, $mail_title, $mail_text ) )
					{
						...
						$mail_title = pkGetSpecialLang( 'registration_mail_notify_title', pkGetConfig( 'site_name' ), $newuser );

						$mail_text = pkGetSpecialLang( 'registration_mail_notify_body', pkGetConfig( 'site_name' ), $newuser, $newemail, $info, $link );
