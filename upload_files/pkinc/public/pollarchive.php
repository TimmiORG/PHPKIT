...
//presets
$rowimage = $vote_row_left = $info_msg = $vote_description = $vote_row_result = $vote_row_right = $vote_row = $archiv_closed = $vote_row_other = $archiv_row_vote = $vote_button = '';
	...
	if ( $voteid && $vid )
	{
		...
		if ( getrights( $voteinfo[ 'votetheme_rights' ] ) == 'true' && $voteinfo[ 'votetheme_status' ] == 1 )
		{
			...
			if ( $counter[ 0 ] < 1 )
			{
				...
				$votetotal = $SQL->fetch_array( $SQL->query( "SELECT SUM(vote_counter) FROM " . pkSQLTAB_POLL . " WHERE vote_themeid='" . intval(
					$voteinfo[ 'votetheme_id' ] ) . "'" ) );

				if ( ( $votetotal[ 0 ] >= $voteinfo[ 'votetheme_maxvotes' ] &&
				       $voteinfo[ 'votetheme_maxvotes' ] > 0 ) || (
						$voteinfo[ 'votetheme_expire' ] != 0 && $voteinfo[ 'votetheme_expire' ] <= time( ) ) )
				{
...
while ( $voteinfo = $SQL->fetch_array( $getvote ) )
{
	...
	$votetotal = $SQL->fetch_array( $SQL->query( "SELECT sum(vote_counter) FROM " . pkSQLTAB_POLL . " WHERE vote_themeid='" .
	                                             $voteinfo[ 'votetheme_id' ] . "'" ) );
	...
	{
		$getvoteanswer = $SQL->query( "SELECT * FROM " . pkSQLTAB_POLL . " WHERE vote_themeid='" .
		                              $voteinfo[ 'votetheme_id' ] . "' ORDER by vote_order ASC" );
		...
		while ( $voteanswerinfo = $SQL->fetch_array( $getvoteanswer ) )
		{
			...
			if ( $voteinfo[ 'votetheme_hidderesult' ] != 1 )
			{
				if ( $votetotal[ 0 ] >= 1 )
				{
					$vote_percent = number_format( ( ( $voteanswerinfo[ 'vote_counter' ] * 100 ) /
					                                 $votetotal[ 0 ] ), 0, ",", "." );
				}
				else
				{
		...
		if ( $voteinfo[ 'votetheme_comment' ] == 1 )
		{
			...
			list( $comment_count ) = $SQL->fetch_row( $SQL->query( "SELECT COUNT(comment_id) FROM " . pkSQLTAB_COMMENT . " WHERE comment_cat='vote' AND comment_subid='" .
			                                                       $voteinfo[ 'votetheme_id' ] . "'" ) );
		...
		$vote_row_result = $vote_button = $archiv_row_vote = $vote_comment = $vote_result = $vote_description = $info_msg = '';
	}
