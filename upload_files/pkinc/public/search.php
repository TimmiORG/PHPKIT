...
switch($mode)
	...
	case 'result' :
		...
		if((empty($_POST['search_text']) && !$SESSION->exists('save_rcontent')) || (isset($_POST['search']) && isset($_POST['search_text']) && $ACTION==$_POST['search'] && empty($_POST['search_text'])))
			{
		...
		if(!empty($search_text))
			...
			$search_string=explode(" or ",strtolower(eregi_replace("\+"," AND ",eregi_replace("\-"," OR ",trim($search_text)))));
			...
			if(isset($_POST['search']) || $ACTION==$_POST['search'])
				...
				$getcontent=$SQL->query("SELECT 
					...
					(content_expire>'".pkTIME."' OR content_expire=0) ".
					$order."
	...
	default :
		...
		if(is_array($contentcat_cache))
			...
			foreach($contentcat_cache as $contentcat)
				...
				$cat_option.='<option value="'.$contentcat['contentcat_id'].'">'.pkEntities($contentcat['contentcat_name']).'</option>';
				}
			}
