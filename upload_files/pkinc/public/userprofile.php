...
$buddy_cache = $profile_info = $profile_gbook = $profil_favorits = $bd_option_day = $bd_option_month = $avatar_deselect = $avatar_upload = $avatar_show = $im_option0 = $im_imnotify0 = $nl_option0 = $im_sigoption0 = $im_option1 = $im_imnotify1 = $nl_option1 = $im_sigoption1 = $ghost_eod = $avatar_list = $avatar_uploadlink = $avatar_uploadlink = $buddy_head = '';

$modehash = array(
	'avatar', 'delete', 'edit', 'friends', 'options'
);
...
switch ( $mode )
{
	case 'avatar' :
		...
		if ( !empty($_REQUEST[ 'upload' ]) && ( $_REQUEST[ 'upload' ] == 1 && $config[ 'avatar_eod' ] == 1 ) || (
				$config[ 'avatar_eod' ] != 1 && $config[ 'avatar_eod' ] != 2 ) )
		{
		...
		if ( !empty( $_REQUEST[ 'upload' ] ) )
		{
			if ( $ACTION == $_POST[ 'upload_action' ] && is_uploaded_file( $_FILES[ 'upload_pic' ][ 'tmp_name' ] ) )
			{
				...
				if ( isset( $ext ) && isset( $avatar_size ) )
				{
					...
					if ( ( $config[ 'avatar_height' ] < $avatar_size[ 1 ] ) || (
							$config[ 'avatar_width' ] < $avatar_size[ 0 ] ) )
					{
						$error = 1;
					}
			...
			{
				$error = ( isset( $_REQUEST[ 'error' ] ) && intval( $_REQUEST[ 'error' ] ) > 0 && intval(
					                                                                                  $_REQUEST[ 'error'
					                                                                                  ] ) < 5 ) ? intval(
					$_REQUEST[ 'error' ] ) : 0;
	...
	case 'edit' :
		...
		$bd_option_year = $uderror = $editprofile_userfield = $user_email_option_1 = $user_email_option_0 = $event = $user_sex_option_1 = $user_sex_option_2 = $user_sex_option_0 = NULL;
		...
		{
			$uderror = NULL;
			$ud_nick = ( !empty( $_POST[ 'ud_nick' ] ) ) ? trim($_POST[ 'ud_nick' ] ) : 'NICHT LEER';
			$ud_newpw1 = ( !empty( $_POST[ 'ud_newpw1' ] ) ) ? trim($_POST[ 'ud_newpw1' ] ) : '';
			$ud_newpw2 = ( !empty( $_POST[ 'ud_newpw2' ] ) ) ? trim($_POST[ 'ud_newpw2' ] ) : '';
			$ud_email = ( !empty( $_POST[ 'ud_email' ] ) ) ? trim(strtolower($_POST[ 'ud_email' ] ) ) : '';
			$cur_password = ( isset( $_POST[ 'cur_password' ] ) && !empty( $_POST[ 'cur_password' ] ) ) ? md5( trim($_POST['cur_password'] ) ) : '';
/*
			if(empty($ud_nick))
			{
				die('$ud_nick IST LEER');
			}
			elseif(trim($ud_nick) == '')
			{
				die('$ud_nick IST NACH TRIM LEER!');
			}
			else
			{
				die('$ud_nick IST NICHT LEER = '. $ud_nick);
			}
			*/
			...
			$vars = array(
				'ud_nick' => 'ud_nick', 'ud_hpage' => 'ud_hpage', 'ud_aimid' => 'ud_aimid', 'ud_yim' => 'ud_yim',
				'ud_icqid' => 'ud_icqid', 'ud_usersig' => 'ud_usersig', 'ud_userqou' => 'ud_userqou',
				'ud_userhobby' => 'ud_userhobby', 'ud_sex' => 'ud_sex', 'ud_emailshow' => 'ud_emailshow',
				'ud_country' => 'ud_country', 'user_bd_day' => 'user_bd_day', 'user_bd_month' => 'user_bd_month',
			...
			$array = array(
				'nick' => 'nick', 'pass' => 'pw', 'email' => 'email', 'sex' => 'sex', 'hpage' => 'hpage',
				'icqid' => 'icqid'
			);
		...
		foreach ( range( 1, 31 ) as $d )
			...
			$info[ 'user_bd_day' ] == $d ? ' selected="selected"' : '' ) . '>' . $d . '</option>';
		...
		foreach ( range( date( 'Y' ) - 5, 1900 ) as $y )
			$bd_option_year .= '<option ' . (
			$info[ 'user_bd_year' ] == $y ? ' selected="selected"' : '' ) . '>' . $y . '</option>';
		...
		foreach ( array(
			'edit_profile', 'username', 'username_description', 'userstatus', 'userstatus_description', 'member_since',
			'account_information', 'current_password', 'current_password_description', 'nickname',
			'nickname_description', 'password', 'password_description', 'confirm_password',
			'confirm_password_description', 'email_description', 'optional_specifications', 'show_email',
			'show_email_description', 'sex', 'sex_description', 'sex_not_specified', 'sex_male', 'sex_female',
			'dateofbirth', 'dateofbirth_description', 'dateofbirth_day', 'dateofbirth_month', 'dateofbirth_year',
			'origin', 'origin_description', 'origin_ger', 'origin_aut', 'origin_ch', 'origin_nl', 'origin_oth',
			'origin_def', 'homepage', 'homepage_description', 'aim_screenname', 'aim_screenname_description', 'yim',
			'yim_description', 'icq', 'icq_description', 'signature', 'signature_description', 'quotation',
			'quotation_description', 'hobbies', 'hobbies_description'
		) as $l )
		{
			$v = 'L_editprofile_' . $l;
			$$v = pkGetLang( $l );
	...
	case 'options' :
		...
		if ( !empty( $info[ 'user_profillock' ] ) && $info[ 'user_profillock' ] == 1 )
		{
		...
		if ( isset( $_REQUEST[ 'setavatar' ] ) )
		{
			...
			if ( ( !pkFileCheck( $path ) ) || ( substr( $path, -4 ) == '.php' ) || ( strstr( $avatar, 'avauser' ) && ( substr( $avatar, 0, 8 + strlen( pkGetUservalue( 'id' ) ) ) != 'avauser_' . pkGetUservalue( 'id' ) || pkGetConfig( 'avatar_eod' ) != 2 ) ) || ( strstr( $avatar, 'avamember' ) && ( pkGetUservalue( 'status' ) != 'member' && pkGetUservalue( 'status' ) != 'mod' && pkGetUservalue( 'status' ) != 'admin' ) ) || ( strstr( $avatar, 'avamod' ) && ( pkGetUservalue( 'status' ) != 'mod' && pkGetUservalue( 'status' ) != 'admin' ) ) || ( strstr( $avatar, 'avaadmin' ) && pkGetUservalue( 'status' ) != 'admin' ) )
			{
				$avatar = '';
			}
		...
		if ( $userinfo[ 'user_imoption' ] == 1 )
		{
			$im_option1 = ' checked="checked"';
		}
		else
		{
			$im_option0 = ' checked="checked"';
		}

		if ( $userinfo[ 'user_imnotify' ] == 1 )
		{
			$im_imnotify1 = ' checked="checked"';
		}
		else
		{
			$im_imnotify0 = ' checked="checked"';
		}

		if ( $userinfo[ 'user_sigoption' ] == 1 )
		{
			$im_sigoption1 = ' checked="checked"';
		}
		else
		{
			$im_sigoption0 = 'checked="checked"';
		}

		if ( $userinfo[ 'user_nloption' ] == 1 )
		{
			$nl_option1 = ' checked="checked"';
		}
		else
		{
			$nl_option0 = ' checked="checked"';
		}
		...
		if ( $config[ 'user_ghost' ] == 1 )
		{
			...
			{
				$ghost_option1 = ' checked="checked"';
			}
			else
			{
				$ghost_option0 = ' checked="checked"';
			}
		...
		{
			$userdesign = $userinfo[ 'user_design' ] ? $userinfo[ 'user_design' ] : pkGetConfig( 'site_style' );
			...
			{
				$style_option .= '<option value="' . $styleinfo[ 'style_id' ] . '"' . ( ( $userdesign == $styleinfo[ 'style_id'] ) ? ' selected="selected"' : '' ) . '>' . pkEntities($styleinfo[ 'style_name' ] ) . '</option>';
			}
	...
	default :
		...
		while ( $buddy = $SQL->fetch_array( $getbuddies ) )
		{
			...
			if ( !$sqlcommand )
			{
				$sqlcommand = "SELECT user_imoption, user_nick, user_id, user_ghost FROM " . pkSQLTAB_USER . " WHERE user_id='" .
