...
//presets
$order0 = $order1 = $set = $imcenter_new = $im_nextnew = $im_viewtime = $im_type = $info_sig = $sign_format = $write_error = $savemessage = $im_autor = $iminfo_receive_hash = $im_title = $iminfo_text = '';
...
$imid = ( isset( $_REQUEST[ 'imid' ] ) && intval( $_REQUEST[ 'imid' ] ) > 0 ) ? intval(
	$_REQUEST[ 'imid' ] ) : ( ( isset( $_REQUEST[ 'imid' ] ) && $_REQUEST[ 'imid' ] == 'new' ) ? 'new' : 0 );
$view = ( isset( $_REQUEST[ 'view' ] ) ? ( intval( $_REQUEST[ 'view' ] ) > 0 ? intval( $_REQUEST[ 'view' ] ) : (
$_REQUEST[ 'view' ] == 'receive' || $_REQUEST[ 'view' ] == 'send' ? $_REQUEST[ 'view' ] : 'all' ) ) : 'all' );

pkLoadFunc( 'user' );
$user_navigation = pkUserNavigation( );

if( !empty( $_REQUEST[ 'userid' ] ) && intval( $_REQUEST[ 'userid' ] ) > 0 )
{
	$_REQUEST[ 'writeim' ] = 1;
}
...
elseif( !empty( $_POST[ 'delete' ] ) && $ACTION == $_POST[ 'delete' ] )
{
	...
	if( isset( $imid ) && !empty( $_REQUEST[ 'deltype' ] ) && $_REQUEST[ 'deltype' ] == 'to' )
	{
		$SQL->query( "UPDATE " . pkSQLTAB_USER_PRIVATEMESSAGE . " SET im_del=1 WHERE im_to='" . $SQL->i( pkGetUservalue( 'id' ) ) . "' AND im_id='" . $imid . "'" );
	}

	if( isset( $imid ) && !empty( $_REQUEST[ 'deltype' ] ) && $_REQUEST[ 'deltype' ] == 'autor' )
	{
...
if( mt_rand( 1, 20 ) == 7 )
{
	$SQL->query( "DELETE FROM " . pkSQLTAB_USER_PRIVATEMESSAGE . " WHERE (im_del=1 AND im_delautor=1) " . ( pkGetConfig( 'user_pndelete' ) ? " OR im_time<'" . ( pkTIME - pkGetConfig( 'user_pndelete' ) * 86400 ) . "'" : '' ) );
}
...
if( $imid )
{
	...
	if( $iminfo = $SQL->fetch_assoc( $SQL->query( "SELECT * FROM " . pkSQLTAB_USER_PRIVATEMESSAGE . " WHERE " . $sqlcommand . " LIMIT 1" ) ) )
	{
		...
		else
		{
			...
			if( $iminfo[ 'im_view' ] != 1 )
			{
				$SQL->query( "UPDATE " . pkSQLTAB_USER_PRIVATEMESSAGE . " SET im_view=1, im_viewtime='" . pkTIME . "' WHERE im_id='" .
				             $iminfo[ 'im_id' ] . "'" );
			}
		...
		$im_text = $BBCODE->parse( $iminfo[ 'im_text' ], 0, $config[ 'text_ubb' ], $config[ 'text_smilies' ],
		                           $config[ 'text_images'
		                           ], 1, pkGetConfig( 'user_imageresize' ), pkGetConfig( 'user_textwrap' ) );
		...
		$lang_pn_message_title =
				$iminfo[ 'im_to' ] == $userid ? $lang[ 'private_message_sent' ] : $lang[ 'private_message_received' ];
...
{
	if( !empty( $_POST[ 'send' ] ) && $ACTION == $_POST[ 'send' ] )
	{
		pkLoadLang( 'error' );
			...
			if( $userinfo[ 'user_imoption' ] == 1 )
			{
				if( trim( $im_title ) != '' && trim( $im_text ) != '' )
				{
					...
					$SQL->query( "INSERT INTO " . pkSQLTAB_USER_PRIVATEMESSAGE . "
						...
						('" . $userinfo[ 'user_id'
					] . "','" . $SQL->i( pkGetUservalue( 'id' ) ) . "','" . $SQL->f( $im_title ) . "','" . $SQL->f( $im_text ) . "',
							'" . pkTIME . "','" . $delmessage . "')" );
					...
					{
						mailsender( $userinfo[ 'user_email' ],
						            $config[ 'site_name' ] . ': ' . $lang[ 'new_instantmessage' ], $imnotify_text );
					}
	...
	if( !empty( $_REQUEST[ 'reply' ] ) && intval( $_REQUEST[ 'reply' ] ) > 0 )
	{
		if( $iminfo = $SQL->fetch_assoc( $SQL->query( "SELECT im_text, im_time, im_autor, im_title
			FROM " . pkSQLTAB_USER_PRIVATEMESSAGE . "
			WHERE im_to='" . $SQL->i( pkGetUservalue( 'id' ) ) . "' AND im_id='" . $SQL->i(
			$_REQUEST[ 'reply' ] ) . "' LIMIT 1" ) ) )
		{
	...
	if( !empty( $_REQUEST[ 'postid' ] ) && !empty( $_REQUEST[ 'threadid' ] ) && intval( $_REQUEST[ 'postid'
	                                                                                    ] ) > 0 && intval( $_REQUEST[
	                                                                                                       'threadid'
	                                                                                                       ] ) > 0 )
	{
		$im_title = pkGetLang( 'report_forumsposting_title' );
		$iminfo_text = '[url=include.php?path=forumsthread&threadid=' . intval(
			$_REQUEST[ 'threadid' ] ) . '&postid=' . intval(
			$_REQUEST[ 'postid' ] ) . ']' . pkGetLang( 'report_forumsposting_namelink' ) . '[/url]';
	}

	if( !empty( $_REQUEST[ 'userid' ] ) && intval( $_REQUEST[ 'userid' ] ) > 0 )
	{
		$userinfo = $SQL->fetch_array( $SQL->query( "SELECT user_nick FROM " . pkSQLTAB_USER . " WHERE user_id='" . intval(
			$_REQUEST[ 'userid' ] ) . "' LIMIT 1" ) );
		$im_autor = $userinfo[ 'user_nick' ];
	}

	if( !empty( $_POST[ 'im_receiver' ] ) )
	{
		$_POST[ 'im_receiver' ] = trim( $_POST[ 'im_receiver' ] );
	}
	...
	if( !empty( $_POST[ 'im_title' ] ) )
	{
		$_POST[ 'im_title' ] = trim( $_POST[ 'im_title' ] );
	}
	if( empty( $_POST[ 'im_title' ] ) )
	{
		$im_title = $im_title;
	...
	if( !empty( $_POST[ 'im_savemessage' ] ) && $_POST[ 'im_savemessage' ] == 1 )
	{
		$savemessage = 'checked';
	}
...
else
{
	...
	if( !empty( $_REQUEST[ 'order' ] ) && $_REQUEST[ 'order' ] == 'time' )
	{
		$order1 = "up";
		$order = "ASC";
	}
	...
	$getiminfo = $SQL->query( "SELECT
			*
			...
			WHERE ((im_to='" . $SQL->i( pkGetUservalue( 'id' ) ) . "' AND im_del<>1) OR
				(im_autor='" . $SQL->i( pkGetUservalue( 'id' ) ) . "' AND  im_delautor<>1))" . ( pkGetConfig( 'user_pndelete' ) ? " AND im_time>'" . ( pkTIME - pkGetConfig( 'user_pndelete' ) * 86400 ) . "'" : '' ) . "
	...
	while( $iminfo = $SQL->fetch_array( $getiminfo ) )
	{
		if( $iminfo[ 'im_to' ] == pkGetUservalue( 'id' ) && $iminfo[ 'im_del' ] != 1 )
		{
			...
			if( $iminfo[ 'im_autor' ] > 0 )
			{
				...
				{
					$sqlcommand = "SELECT * FROM " . pkSQLTAB_USER . " WHERE user_id='" . $SQL->i(
						$iminfo[ 'im_autor' ] ) . "'";
				}
			}
		}
	...
	if( !empty( $sqlcommand ) && $sqlcommand )
	{
	...
	if( $view != 'receive' )
	{
		...
		if( !empty( $iminfo_send_hash ) && is_array( $iminfo_send_hash ) )
		{
	...
	$lang_pncenter_message = pkGetLang( 'pncenter_message' ) . pkGetSpecialLang( 'pncenter_message_delete', pkGetConfig( 'user_pndelete' ) );
