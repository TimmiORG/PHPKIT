...
$modehash = array(
	'firstlog', 'lostpassword', 'newpassword'
);
...
switch( $mode )
{
	case 'lostpassword' :
		...
		if( !empty( $_POST[ 'submit' ] ) && $ACTION == $_POST[ 'submit' ] )
		{
			if( !empty( $_POST[ 'lostem' ] ) && emailcheck( $_POST[ 'lostem' ] ) )
			{
				pkLoadLang( 'email' );
				...
				$email = $SQL->fetch_assoc( $SQL->query( "SELECT
					...
					WHERE user_email='" . $SQL->f( strtolower( $_POST[ 'lostem' ] ) ) . "'
					LIMIT 1" ) );
				...
				if( $email[ 'user_activate' ] == 1 && !empty( $email[ 'user_email' ] ) )
				{
					...
					$mail_title = pkGetSpecialLang( 'lostpassword_mail_title', pkGetConfig( 'site_name' ) );

					$mail_text = pkGetSpecialLang( 'lostpassword_mail_text',
					                               $email[ 'user_nick' ], pkGetConfig( 'site_name' ), $link,
					                               $email[ 'user_name'
					                               ], pkGetConfig( 'site_name' ), pkGetConfig( 'site_url' ) );

					if( mailsender( mailalias(
						                $email[ 'user_email' ], $email[ 'user_nick' ] ), $mail_title, $mail_text ) )
					{
		...
		elseif( !empty( $_POST[ 'cancel' ] ) && $ACTION == $_POST[ 'cancel' ] )
		{
			pkHeaderLocation( 'start' );
		}
		else
		{
			$error = ( isset( $_GET[ 'error' ] ) && intval( $_GET[ 'error' ] ) > 0 && intval( $_GET[ 'error'
			                                                                                  ] ) < 4 ) ? intval(
				$_GET[ 'error' ] ) : '';
	...
	case 'newpassword' :
		...
		$ACTION = ( isset( $_POST[ 'action' ] ) && isset( $_POST[ 'change' ] ) &&
		            $_POST[ 'action' ] == $_POST[ 'change' ] ) ? 'change' : '';
		...
		if( $ACTION == 'change' )
		{
			$newpassword = ( isset( $_POST[ 'newpassword' ] ) && !empty( $_POST[ 'newpassword' ] ) ) ?
					$_POST[ 'newpassword' ] : '';
			$newpassword2 = ( isset( $_POST[ 'newpassword2' ] ) && !empty( $_POST[ 'newpassword2' ] ) ) ?
					$_POST[ 'newpassword2' ] : '';
