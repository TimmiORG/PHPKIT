...
//preset
$sqlcommand = $sqlcommand  = $row = $info_sig = $gbook_email = $gbook_hpage = $gbook_icq = $gbook_row = $autor_message = $sign_comment = $gbook_icqnr = $gbook_title = $gbook_text = $gbook_time = $gb_uid = '';
$modehash = array(
	'delete', 'edit', 'print', 'sign'
);
...
switch ( $mode )
{
	case 'delete' :
		...
		if ( intval( $gbookinfo[ 'gbook_userid' ] ) > 0 )
		{
			...
			if ( intval( $userinfo[ 'expire' ] ) > 0 && ( $config[ 'user_ghost' ] != 1 || (
					$config[ 'user_ghost' ] == 1 && $userinfo[ 'user_ghost' ] != 1 ) ) )
			{
		...
		else
		{
			...
			{
				if ( stripos( "http://", $gbookinfo[ 'gbook_hpage' ] ) !== FALSE )
				{
		...
		$gbook_text = $BBCODE->parse( $gbookinfo[ 'gbook_text' ], 0, $config[ 'gbook_ubb' ], $config[ 'gbook_smilies' ],
		                              $config[ 'gbook_images'
		                              ], 1, pkGetConfig( 'guestbook_imageresize' ), pkGetConfig( 'guestbook_textwrap' ) );
		...
		{
			list( $ccounter ) = $SQL->fetch_row( $SQL->query( "SELECT COUNT(*) FROM " . pkSQLTAB_COMMENT . " WHERE comment_cat='gb' and comment_subid='" .
			                                                  $gbookinfo[ 'gbook_id' ] . "'" ) );
	...
	case 'edit' :
		...
		$link_administration_useredit = pkLinkAdmin( '', '', 'goto=' . urlencode( 'path=useredit&editid=' .
		                                                                          $gbookinfo[ 'gbook_userid' ] ) );
	...
	case 'sign' :
		...
		if ( ( isset( $_POST[ 'save' ] ) && $ACTION == $_POST[ 'save' ] ) || ( isset(
		                                                                       $_POST[ 'preview' ] ) && $ACTION ==
		                                                                                                $_POST[
		                                                                                                'preview' ] ) )
		{
			...
			elseif ( ( isset( $_POST[ 'content' ] ) && empty( $_POST[ 'content' ] ) ) || ( isset( $_POST[ 'gbook_title'
			] ) && empty( $_POST[ 'gbook_title' ] ) ) || ( isset( $_POST[ 'gbook_autor' ] ) && empty(
			$_POST[ 'gbook_autor' ] ) ) )
			{
				$error = 1;
			}
			...
			elseif ( ( $_POST[ 'gbook_notify' ] == 1 || $_POST[ 'gbook_email' ] != '' ) && !emailcheck(
				$_POST[ 'gbook_email' ], 1 ) )
			{
				$error = 5;
			}
			else
			{
				if ( $ACTION == $_POST[ 'save' ] )
				{
						 ...
						 '" . $SQL->i( $_POST[ 'gbook_notify' ] ) . "')" ) )
					{
						...
						$mail_title = $config[ 'site_name' ] . ' - ' . pkGetLang( 'new_gbentry' ) . ': ' .
						              $_POST[ 'gbook_title' ];
						$mail_text = pkGetSpecialLang( 'guestbook_notify_mail_text', pkGetConfig( 'site_name' ), $autor, $link );
				...
				elseif ( $ACTION == $_POST[ 'preview' ] )
				{
					...
					{
						if ( stripos( "http://", $_POST[ 'gbook_hpage' ] ) !== FALSE )
						{
					...
					$gbook_text = $BBCODE->parse(
						$_POST[ 'content' ], 0, $config[ 'gbook_ubb' ], $config[ 'gbook_smilies' ],
						$config[ 'gbook_images'
						], 1, pkGetConfig( 'guestbook_imageresize' ), pkGetConfig( 'guestbook_textwrap' ) );
		...
		if ( !empty($_POST[ 'gbook_check' ]) && $_POST[ 'gbook_check' ] == 1 )
		{
			$check1 = "checked";
		}
		elseif ( $ACTION == 'view' )
		...
		if ( !empty($_POST[ 'gbook_commenteod' ]) && $config[ 'gbook_commenteod' ] == 1 )
		{
		...
		if ( !empty( $_POST[ 'gbook_icqnr' ] ) && intval( $_POST[ 'gbook_icqnr' ] ) > 0 )
		{
		...
		if ( !empty($_POST[ 'gbook_title' ] ) && trim($_POST[ 'gbook_title' ]) != '')
		{
			$gbook_title = pkEntities( $_POST[ 'gbook_title' ] );
		}
		...
		if ( !empty( $_POST[ 'gbook_ubb' ] ) && $config[ 'gbook_ubb' ] == 1 )
		{
			eval( "\$sign_format= \"" . pkTpl( "format_text" ) . "\";" );
		}

		if ( !empty( $_POST[ 'gbook_smilies' ] ) && $config[ 'gbook_smilies' ] == 1 )
		{
			$smilies = new smilies( );
	...
	default :
		...
		{
			$counter = $SQL->fetch_array( $SQL->query( "SELECT COUNT(*) FROM " . pkSQLTAB_GUESTBOOK ) );
			$sidelink = sidelinkfull(
				$counter[ 0 ], $config[ 'gbook_epp' ], $entries, "include.php?path=guestbook", "small" );
		...
		if ( !empty( $_REQUEST[ 'gbid' ] ) && intval( $_REQUEST[ 'gbid' ] ) > 0 )
		{
		...
		$getinfo = $SQL->query( "SELECT * FROM " . pkSQLTAB_GUESTBOOK . " " . $sqlcommand . " ORDER BY gbook_time DESC LIMIT " . $entries . "," . $config[ 'gbook_epp' ] );
		$sqlcommand = '';
		...
		while ( $gbookinfo = $SQL->fetch_array( $getinfo ) )
		{
			...
			if ( $gbookinfo[ 'gbook_userid' ] != 0 )
			{
				...
				{
					$sqlcommand = "SELECT * FROM " . pkSQLTAB_USER . " WHERE user_id='" .
					              $gbookinfo[ 'gbook_userid' ] . "'";
				}
			}
		}
		...
		if ( is_array( $gbookinfo_hash ) )
		{
			foreach ( $gbookinfo_hash as $gbookinfo )
			{
				...
				$gbook_title = pkEntities( $gbookinfo[ 'gbook_title' ] );
				$gbook_text = $BBCODE->parse(
					$gbookinfo[ 'gbook_text' ], 0, $config[ 'gbook_ubb' ], $config[ 'gbook_smilies' ],
					$config[ 'gbook_images'
					], 1, pkGetConfig( 'guestbook_imageresize' ), pkGetConfig( 'guestbook_textwrap' ) );
				$gbook_time = formattime( $gbookinfo[ 'gbook_time' ] );
