...
$id = ( isset( $_REQUEST[ 'id' ] ) && intval( $_REQUEST[ 'id' ] ) > 0 ) ? intval(
	$_REQUEST[ 'id' ] ) : pkGetUservalue( 'id' );
...
if ( $userinfo[ 'user_bd_day' ] != 0 && $userinfo[ 'user_bd_month' ] != 0 && $userinfo[ 'user_bd_year' ] != 0 )
{
	...
	$info_birthday .= $userinfo[ 'user_bd_year' ] . " - " . $lang[ 'age' ] . ": " . getAge(
		$userinfo[ 'user_bd_day' ], $userinfo[ 'user_bd_month' ], $userinfo[ 'user_bd_year' ] );
}
else
{
...
$info_logtime =
		$userinfo[ 'logtime' ] ? formattime( $userinfo[ 'logtime' ] ) : formattime( $userinfo[ 'signin' ], '', 'date' );
...
{
	$group = $SQL->fetch_array( $SQL->query( "SELECT usergroup_name FROM " . pkSQLTAB_USER_GROUP . " WHERE usergroup_id='" .
	                                         $userinfo[ 'user_groupid' ] . "' LIMIT 1" ) );
...
$userfields = $SQL->fetch_array( $SQL->query( "SELECT * FROM " . pkSQLTAB_USER_FIELDS . " WHERE userid='" .
...
{
	$info_qoute = $BBCODE->parse( $userinfo[ 'user_qou' ], 0, $config[ 'text_ubb' ], $config[ 'text_smilies' ],
	                              $config[ 'text_images'
	                              ], 1, pkGetConfig( 'user_imgresize' ), pkGetConfig( 'user_textwrap' ) );
}
else
{
...
{
	$info_hobby = $BBCODE->parse( $userinfo[ 'user_hobby' ], 0, $config[ 'text_ubb' ], $config[ 'text_smilies' ],
	                              $config[ 'text_images'
	                              ], 1, pkGetConfig( 'user_imgresize' ), pkGetConfig( 'user_textwrap' ) );
}
else
...
$info_sig = empty( $userinfo[ 'user_sig' ] ) ? pkGetLang( 'not_specified' ) : pkUserSignature(
	$userinfo[ 'user_sig' ], 0 );
...
if ( $config[ 'avatar_eod' ] != 0 )
{
	...
	if ( !empty( $userinfo[ 'user_avatar' ] ) && @filecheck(
...
if ( $config[ 'forum_eod' ] == 1 )
{
	...
	if ( $info_userposts > 0 )
	{
		...
		$threadcount = $SQL->fetch_array( $SQL->query( "SELECT COUNT(*) FROM " . pkSQLTAB_FORUM_THREAD . " WHERE forumthread_autorid='" .
		                                               $userinfo[ 'user_id' ] . "'" ) );
		...
		if ( !empty( $lastpost[ 0 ] ) )
		{
			$forumthread = $SQL->fetch_array( $SQL->query( "SELECT forumthread_id, forumthread_title, forumthread_catid FROM " . pkSQLTAB_FORUM_THREAD . " WHERE forumthread_id='" .
			                                               $lastpost[ 'forumpost_threadid' ] . "'" ) );
			$forumcat = $SQL->fetch_array( $SQL->query( "SELECT forumcat_id, forumcat_name FROM " . pkSQLTAB_FORUM_CATEGORY . " WHERE forumcat_id='" .
			                                            $forumthread[ 'forumthread_catid' ] . "'" ) );
			...
			$link_thread = pkLink( 'forumsthread', '', 'threadid=' . $forumthread[ 'forumthread_id' ] . '&postid=' .
			                                           $lastpost[ 'forumpost_id' ] );
