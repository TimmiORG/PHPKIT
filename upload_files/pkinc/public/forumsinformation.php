...
$finfoid=(isset($_REQUEST['finfoid']) && intval($_REQUEST['finfoid'])>0) ? intval($_REQUEST['finfoid']) : 0;
...
elseif(!(getrights($forumcat['forumcat_rrights'])=="true" || userrights($forumcat['forumcat_mods'],$forumcat['forumcat_rrights'])=="true" || userrights($forumcat['forumcat_user'],$forumcat['forumcat_rrights'])=="true"))
	...
	if(!$userinfo=$SQL->fetch_array($SQL->query("SELECT * FROM ".pkSQLTAB_USER." WHERE user_id='".$foruminfo['foruminfo_autorid']."' LIMIT 1")))
		{
