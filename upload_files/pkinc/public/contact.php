...
$error_message = $mailer_msg = $mailer_title = $mailer_text = '';
...
switch ( $mode )
{
	case 'user' :
		...
		$userid = isset( $_REQUEST[ 'userid' ] ) && intval( $_REQUEST[ 'userid' ] ) ? intval(
			$_REQUEST[ 'userid' ] ) : 0;
		...
		if ( !empty($_POST[ 'send' ]) && $ACTION == $_POST[ 'send' ] && $_POST[ 'mailer_title' ] != "" && $_POST[ 'mailer_text' ] != "" )
		{
		...
		if ( !empty($_POST[ 'action' ]) && $_POST[ 'action' ] == $LANG[ 'send' ] )
		{
	...
	case 'suggest' :
		...
		if ( !empty($_POST[ 'send' ]) && $ACTION == $_POST[ 'send' ] && ( $captcha_check = pkCaptchaCodeValid( $ENV->_post( pkCAPTCHAVARNAME ) ) ) )
		{
			if ( emailcheck( $_POST[ 'suggest_email' ] ) )
			{
				...
				{
					pkHeaderLocation( '', '', 'event=suggestion_sent' . ( isset(
					$_POST[ 'suggest_url' ] ) ? '&moveto=' . urlencode( $_POST[ 'suggest_url' ] ) : '' ) );
				}
		...
		if ( !empty($_POST[ 'send' ]) && $ACTION == $_POST[ 'send' ] && !$captcha_check )
		{
		...
		$suggest_email = '';
		if ( !empty( $_REQUEST[ 'suggest_email' ] ) && pkEntities( $_REQUEST[ 'suggest_email'] ) != pkGetLang( 'email_address' ) )
		{
			$suggest_email = pkEntities( $_REQUEST[ 'suggest_email' ] );
		}

		$suggest_name = '';
		if ( !empty( $_REQUEST[ 'suggest_name' ] ) && pkEntities($_REQUEST[ 'suggest_name' ] ) != pkGetLang( 'receiver' ) )
		{
	...
	default :
		...
		if ( !empty($_POST[ 'send' ]) && $ACTION == $_POST[ 'send' ] && ( $captcha = pkCaptchaCodeValid( $ENV->_post( pkCAPTCHAVARNAME ) ) ) && emailcheck( $contact_email ) && trim( $contact_name ) != '' && trim( $contact_subject ) != '' && trim( $contact_message ) != '' )
		{
			...
			{
				if ( !empty($_POST[ 'contact_copy' ]) && $_POST[ 'contact_copy' ] == 1 )
				{
