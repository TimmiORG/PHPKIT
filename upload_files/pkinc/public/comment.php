...
// preset
$event = $row = $info_sig = $gbook_number = $gbook_email = $gbook_hpage = $gbook_icq = $gbook_admin = $gbook_ip = $comment_row = $sign_format = '';
$comcathash = array(
	'gb', 'cont', 'user', 'vote'
);

$comcat = ( isset( $_REQUEST[ 'comcat' ] ) && in_array( $_REQUEST[ 'comcat' ], $comcathash ) ) ?
		$_REQUEST[ 'comcat' ] : ( isset( $comcat ) && in_array( $comcat, $comcathash ) ? $comcat : '' );
$subid = ( isset( $_REQUEST[ 'subid' ] ) && intval( $_REQUEST[ 'subid' ] ) > 0 ) ? intval(
	$_REQUEST[ 'subid' ] ) : ( isset( $subid ) && intval( $subid ) > 0 ? intval( $subid ) : 0 );
$ACTION = isset( $_POST[ 'action' ] ) ? $_POST[ 'action' ] : 'view';

if ( adminaccess( 'comment' ) )
{
	$delcomment = ( isset( $_REQUEST[ 'delcomment' ] ) && intval( $_REQUEST[ 'delcomment' ] ) > 0 ) ? intval(
		$_REQUEST[ 'delcomment' ] ) : 0;
...
if ( isset( $_POST[ 'save' ] ) && $ACTION == $_POST[ 'save' ] )
{
	...
	else
	{
		...
		$mail_text = pkGetSpecialLang( 'comment_mail_notify', pkGetConfig( 'site_name' ), $autor, $link );

		notifymail( 'comment', $mail_title, $mail_text );

		$pn_title = pkGetLang( 'new_comment' );
		$pn_text = pkGetSpecialLang( 'new_comment_pn_notify', $autor, $current_path );
		...
		if ( $comcat == 'gb' )
		{
			...
			if ( $email[ 'gbook_notify' ] == 1 )
			{
				...
				$email_title = pkGetSpecialLang( 'guestbook_comment_mail_title', pkGetConfig( 'site_name' ) );

				$email_text = pkGetSpecialLang( 'guestbook_comment_mail_text', $email[ 'gbook_autor'
				                                                               ], pkGetConfig( 'site_name' ), $comment_autor, $comment_text, pkGetConfig( 'site_name' ), $link, pkGetConfig( 'site_name' ), pkGetConfig( 'site_url' ) );
...
if ( $comcat == "gb" )
{
	...
	while ( $gbookinfo = $SQL->fetch_array( $getinfo ) )
	{
		...
		$userinfo = $SQL->fetch_array( $SQL->query( "SELECT * FROM " . pkSQLTAB_USER . " WHERE user_id='" . $SQL->i(
			$gbookinfo[ 'gbook_userid' ] ) . "' LIMIT 1" ) );
		...
		{
			$ccounter = $SQL->fetch_array( $SQL->query( "SELECT COUNT(*) FROM " . pkSQLTAB_COMMENT . " WHERE comment_cat='gb' and comment_subid='" .
			                                            $gbookinfo[ 'gbook_id' ] . "'" ) );
...
while ( $comment = $SQL->fetch_array( $getcomment ) )
{
	...
	if ( $comment[ 'comment_userid' ] != 0 )
	{
		...
		{
			$sqlcommand = "SELECT user_id, user_nick FROM " . pkSQLTAB_USER . " WHERE user_id='" .
...
if ( is_array( $comment_hash ) )
{
	...
	foreach ( $comment_hash as $comment )
	{
		...
		$comment_text = $BBCODE->parse( $comment[ 'comment_text'
		                                ], 0, pkGetConfig( 'comment_bb' ), pkGetConfig( 'comment_smilies' ), pkGetConfig( 'comment_images' ), 1, pkGetConfig( 'comment_imageresize' ), pkGetConfig( 'comment_textwrap' ) );
...
else
{
	if ( $config[ 'comment_register' ] != 1 || ( $config[ 'comment_register' ] == 1 && pkGetUservalue( 'id' ) ) )
	{
		...
		$comment_text = pkEntities( $content );
		$captcha = pkCaptchaField( );
