...
//presets
$mods = $m = $forumcat_cache = $admin_row = $mod_row = $user_row = $mod_block = $user_block = '';
...
if ( is_array( $forumcat_cache ) )
{
	...
	{
		if ( userrights( $forumcat[ 'forumcat_mods' ], $forumcat[ 'forumcat_rrights' ] ) == "true"
		     or userrights($forumcat['forumcat_user'], $forumcat[forumcat_rrights] ) == "true"
		     or getrights($forumcat['forumcat_rrights'] ) == "true"
		)
		{
...
while ( $userinfo = $SQL->fetch_array( $getmods ) )
{
	...
	else
	{
		...
		{
			if (
				( userrights( $forumcat[ 'forumcat_mods' ], $forumcat[ 'forumcat_rrights' ] ) == "true"
				  or userrights($forumcat['forumcat_user'],$forumcat['forumcat_rrights'] ) == "true"
				  or getrights($forumcat['forumcat_rrights'] ) == "true"
				)
				&& stripos( "-" . $userinfo['user_id'] . "-", $forumcat['forumcat_mods'] ) !== FALSE
			)
			{
