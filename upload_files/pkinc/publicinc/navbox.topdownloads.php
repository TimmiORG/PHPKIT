...
$query=$SQL->query("SELECT 
	...
	WHERE ".pkSQLTAB_CONTENT.".content_option=4 AND
	...
	ORDER by ".pkSQLTAB_CONTENT.".content_rating DESC
	LIMIT 5");
while(list($id,$title)=$SQL->fetch_row($query))
	...
	$boxlinks[]=pkHtmlLink(pkLink('download','','contentid='.$id),$title_cutted,'','pknidtopdownloads'.count($boxlinks),'pkcontent_a_'.$navalign,$title);
	}
