...
$query=$SQL->query("SELECT 
	...
	WHERE ".pkSQLTAB_CONTENT.".content_option=2 AND
...
while(list($id,$title)=$SQL->fetch_row($query))
	...
	$boxlinks[]=pkHtmlLink(pkLink('news','','contentid='.$id),$title_cutted,'','pknidnews'.count($boxlinks),'pkcontent_a_'.$navalign,$title);
	}
