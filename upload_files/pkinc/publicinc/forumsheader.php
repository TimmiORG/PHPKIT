...
$threadid = isset($_REQUEST['threadid']) && intval($_REQUEST['threadid'])>0 ? intval($_REQUEST['threadid']) : 0;
...
if($catid>0)
	...
	if($path=="forumsthread")
		...
		elseif(getrights($forumcat['forumcat_wrights'])=="true" || userrights($forumcat['forumcat_mods'],$forumcat['forumcat_wrights'])=="true" || userrights($forumcat['forumcat_user'],$forumcat['forumcat_wrights'])=="true")
			{
			if((getrights($forumcat['forumcat_trights'])=="true" || userrights($forumcat['forumcat_mods'],$forumcat['forumcat_trights'])=="true" || userrights($forumcat['forumcat_user'],$forumcat['forumcat_trights'])=="true") && $forumcat['forumcat_threads_option']==1)
				{
	...
	elseif($path=="forumscategory")
		...
		elseif((getrights($forumcat['forumcat_trights'])=="true" || userrights($forumcat['forumcat_mods'],$forumcat['forumcat_trights'])=="true" || userrights($forumcat['forumcat_user'],$forumcat['forumcat_trights'])=="true") && $forumcat['forumcat_status']==1 && $forumcat['forumcat_threads_option']==1)
			{
			eval("\$forum_action.= \"".pkTpl("forum/action_thread")."\";");
			}
		}
...
switch($path)
	...
	case 'forumsnewpost': 
		if(intval($threadid)>0)
			...
			$action_type = $lang['forum_new_answer'].' '.$lang['in'].' '.pkEntities($forumthread['forumthread_title']);
			}
		else
			{
